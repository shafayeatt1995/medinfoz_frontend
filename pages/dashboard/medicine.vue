    ...
    <Card>
      <CardHeader>
        <div class="flex justify-between items-start flex-col md:flex-row">
          <div>
            <CardTitle>Medicine list ({{ items }})</CardTitle>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <form @submit.prevent="fetchMedicine" class="space-y-2">
          <div>
          ...
          <div>
            ...
            <Input id="page" placeholder="Enter page number" v-model="form.page" />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      blocked: false,
      loading: false,
      form: { url: "https://medex.com.bd/companies", page: "7" },
    };
  },
  mounted() {
    this.fetchItems();
